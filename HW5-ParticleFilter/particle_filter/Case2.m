...
obs = @(k, xk, vk, j) xk(1,:)*x1(j)/(x2(j)*x3(j)) + ...
    icdf('Normal', r(j,4), 0, abs( xk(2,:) ));   % (returns column vector)
...
% (under the suposition of additive process noise)
p_yk_given_xk = @(k, yk, xk, j) p_obs_noise(yk - obs(k, xk, 0, j), xk);
