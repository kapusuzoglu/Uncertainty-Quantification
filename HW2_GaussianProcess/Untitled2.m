...
%% Training points
n_train = 7;
P = linspace( mu_x1 - dev1*sigma_x1, mu_x1 + dev1*sigma_x1, n_train);
E = linspace( mu_x2 - dev1*sigma_x2, mu_x2 + dev1*sigma_x2, n_train);
I = linspace( muN_x3 - dev1*sigmaN_x3, muN_x3 + dev1*sigmaN_x3, n_train);
...
% Uniform numbers
r = rand(nsamples,3);
P_i = icdf('Extreme Value', r(:,1), mu_x1, sigma_x1);
E_i = icdf('Lognormal', r(:,2), lambda_x2, zeta_x2);
I_i = icdf('Normal', r(:,3), muN_x3, sigmaN_x3);
MCS_disp = P_i./(E_i.*I_i);
%-----------------------------------------------------------------------------------------------------
% End of MCS
% 
...
%     y_star(simulation,1)=k_tp*(k_tt\Y);
% end

xi1 = icdf('Normal',cdf('Extreme Value', P_i, mu_x1, sigma_x1), 0, 1);
xi2 = (log(E_i)-lambda_x2)/zeta_x2;
xi3 = ((I_i-muN_x3)/sigmaN_x3); 

xX = [ones(nsamples,1) xi1 xi2 xi3]';
for i=1:nsamples
%     pred_pt=randn(1,3);
    for j=1:length(Y)
        k_tp(1,j)= (-0.5*( ((xi1(i)-xi(j,1))/x(1))^2 + ...
            ((xi2(i)-xi(j,2))/x(2))^2 + ((xi3(i)-xi(j,3))/x(3))^2));
    end
    k_tp = x(4)^2*exp(k_tp);
%     k_pp=1;
%     pred_pt=[1 pred_pt];
%     y_star(simulation,1)=k_tp*(k_tt\residual)+pred_pt*b;
    y_star(i,1)=k_tp*(k_tt\Y)+xX(:,i)'*b;
end
...
plot(xy,fy,'-r');
hold on
[f1,xp1]=ksdensity(ypred1);
plot(xp1,f1,'--b');
hold on
[f2,xp2]=ksdensity(ypred2);
plot(xp2,f2,'-+g');
hold on
